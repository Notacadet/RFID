...
class updateNomenclature implements updateNomDAO{
	...
	public function updatesNom($categoryName){
		$servername = "localhost";
        $username = "root";
        $password = "sqldba";
        $dbname = "rfid_database";
		...
		$sql = "UPDATE nomenclature SET updated_at = CURDATE() WHERE nomeclature_Name = '$categoryName'";
		$conn->query($sql);
		print $sql;
		echo "Success";
		$conn->close();
