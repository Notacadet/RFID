...
class Model{
	...
	public function createModelView($selectedModel){
		...
		$conn = RfidController::connect();
		$sql = "SELECT items.rfid, items.items_id, items.serialNum, locations.location_id, locations.roomNumber, makes.makeName, models.model_name, models.model_id, users.userName, users.user_id FROM items join locations on items.location_id=locations.location_id join models on items.model_id=models.model_id join makes on models.make_id=makes.make_id join users on items.hrholder_id=users.user_id where models.model_id = '$selectedModel'";
		$result = $conn->query($sql);
		echo "<table>
		<tr>
		<th>RFID</th>
