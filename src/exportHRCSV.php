	...
	public function createCSV($selectedName){
		...
		if ($result->num_rows > 0){
					...
					while($row = $result->fetch_assoc()) {
						fputcsv($file, array($row["rfid"],$row["nomenclature_Name"],$row["roomNumber"],$row["accountedFor"],"itemstatus","","barcode",$row["makeName"],$row["model_Name"],$row["serialNum"],$row["userName"],$row["comments"],"","","",$row["dateAcquired"],$row["price"],$row["pbhrNumber"],""));
					}
				}  
